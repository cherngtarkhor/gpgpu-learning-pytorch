# https://github.com/intel/neural-compressor/blob/master/examples/pytorch/nlp/huggingface_models/language-modeling/quantization/llm/run_clm_no_trainer.py

# python neural_quant_llm.py --model meta-llama/Llama-2-7b-hf --output_dir "./saved_dir" --quantize --approach "weight_only" --performance 
# python neural_quant_llm.py --model meta-llama/Llama-3.2-3B-Instruct --output_dir "./saved_llama3_1b" --quantize --inference --load --approach weight_only --woq_bits 4 --woq_enable_full_range
# python neural_quant_llm.py --model meta-llama/Llama-3.2-1B-Instruct --output_dir "./saved_unsloth" --unsloth --inference --load --approach weight_only --woq_bits 4 --woq_enable_full_range
# python neural_quant_llm.py --model meta-llama/Llama-3.2-1B-Instruct --output_dir "./saved_llama3_1b" --ipex --quantize --inference --load --approach weight_only --woq_bits 4 --woq_enable_full_range
...
parser.add_argument("--save_torch", action="store_true")
parser.add_argument("--load_torch", action="store_true")
...
def get_user_model(model_path = ""):
    ...
    if args.unsloth:
        ...
        user_model, tokenizer = FastLanguageModel.from_pretrained(
        ...
        user_model = AutoModelForCausalLM.from_pretrained(
            model_name,
            torch_dtype = torch.float16, ## new added
...
if args.quantize:
    ...
    if args.approach == 'weight_only':
        print("run weight_only")
        print("args.woq_bits:", args.woq_bits)
        print("args.woq_group_size:", args.woq_group_size)
        print("args.woq_scheme:", args.woq_scheme)
        print("args.woq_algo:", args.woq_algo)
        print("args.enable_mse_search:", args.woq_enable_mse_search)
        print("args.enable_full_range:", args.woq_enable_full_range)
    ...
    # fit : https://github.com/intel/neural-compressor/blob/master/neural_compressor/quantization.py#L33
    # rtn_quantize : https://github.com/intel/neural-compressor/blob/master/neural_compressor/adaptor/pytorch.py#L4578
    ...
    print("save model", args.output_dir)
    q_model.save(args.output_dir)
# https://github.com/intel/neural-compressor/blob/master/test/adaptor/pytorch_adaptor/test_weight_only_adaptor_pytorch.py#L248

    # for name, param in q_model.named_parameters():
    #     print(f"Layer: {name}, Min: {param.min()}, Max: {param.max()}")


saved_dir = "/home/ctkhor/unsloth/unsloth/saved_pretrained"
saved_compressed = "saved_compressed/compressed_model.pt"

if args.export:

    user_model = q_model.export_compressed_model(
...
if args.save_pretrain:
    ...
    # torch.save(user_model.state_dict(), saved_compressed)
    torch.save(user_model, saved_compressed)

if args.save_torch:
    print("save torch model..")
    user_model.save_pretrained(saved_dir)

if args.load_pretrain:
    print("load pretrained model..")
    user_model, tokenizer = get_user_model(saved_dir)

if args.load_torch:
    print("load torch model..")
    model = torch.load(saved_compressed, weights_only=False)

    model.eval()

    # Run inference with the quantized model
    prompt = "Quantized models can"
...
if args.load:
    ...
    if user_model == None:
        print("model is none, loading for saved model..")
        user_model, tokenizer = get_user_model()
    kwargs = {'weight_only': True} if args.approach == 'weight_only' else {}
    print("kwargs:", kwargs)
    print("load:", os.path.abspath(os.path.expanduser(args.output_dir)))
    user_model = load(os.path.abspath(os.path.expanduser(args.output_dir)), user_model, **kwargs)
# https://github.com/intel/neural-compressor/blob/master/neural_compressor/utils/pytorch.py#L210
...
if args.inference:
    if user_model == None:
        print("model is none, loading for inference..")
        user_model, tokenizer = get_user_model()

    # user_model = ipex.llm.optimize(user_model, inplace=True, dtype=torch.float16, device="xpu")

    if args.unsloth:
        FastLanguageModel.for_inference(user_model)
    ...
    user_model = user_model.to("xpu")

#     for name, param in user_model.named_parameters():
#         print(f"Parameter: {name}, Data type: {param.dtype}")

# # The weights should fall within the range defined by the 4-bit quantization scale (e.g., [-8, 7] for signed int4)
#     for name, param in user_model.named_parameters():
#         print(f"Layer: {name}, Min: {param.min()}, Max: {param.max()}")

    for i in range(2):
        start_time = time.time()
        output = user_model.generate(input_ids = inputs, eos_token_id=terminators, max_new_tokens=128, use_cache = True)
